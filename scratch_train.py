...
def train_model(args, model, dataloader, criterion, optimizer, lr_scheduler, writer, device='cpu', val_dataloader=None, start_time=None): 
    ...
    val_images, val_truths_spine,val_truths_dendrite = val_images[:16], val_truths_spine[:16],val_truths_dendrite[:16]
    ...
    for e in range(args.epochs):
        for mb, (images, truths_spine,truths_dendrite) in enumerate(tqdm(dataloader)):
            ...
            model.train()
            model.zero_grad()
            images, truths_spine,truths_dendrite = images.to(device), truths.to(device)
