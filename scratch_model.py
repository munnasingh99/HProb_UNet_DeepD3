...
class HPUNet(nn.Module):
    def __init__(self, in_ch, chs, latent_num=0, out_ch=1, activation="ReLU", scale_depth=None, kernel_size=None, dilation=None, padding_mode='circular', latent_channels=None, latent_locks=None, conv_dim=2):
        ...
        self.decoder_spine = Decoder(list(reversed(chs)), latent_num, activation, list(reversed(scale_depth)), list(reversed(kernel_size)), list(reversed(dilation)), padding_mode=padding_mode, latent_channels=latent_channels, latent_locks=latent_locks, conv_class=self.conv_class)
        self.decoder_head_spine = ScaleBlock(decoder_head_in_channels, out_ch, activation, scale_depth[0], kernel_size[0], dilation[0], padding_mode=padding_mode, conv_class=self.conv_class)

        self.decoder_dendrite = Decoder(list(reversed(chs)), latent_num, activation, list(reversed(scale_depth)), list(reversed(kernel_size)), list(reversed(dilation)), padding_mode=padding_mode, latent_channels=latent_channels, latent_locks=latent_locks, conv_class=self.conv_class)
        self.decoder_head_dendrite = ScaleBlock(decoder_head_in_channels, out_ch, activation, scale_depth[0], kernel_size[0], dilation[0], padding_mode=padding_mode, conv_class=self.conv_class)
    ...
    def forward(self, x, y=None, times=1, first_channel_only=True, insert_from_postnet=False):
        ...
        output_spine = torch.stack(outs_spine, dim=1)
        output_dendrite = torch.stack(outs_dendrite, dim=1)

        if first_channel_only is True:
            output_spine = output_spine[:,:,0]
            output_dendrite = output_dendrite[:,:,0]

        return output_spine,output_dendrite, infodicts_spine,infodicts_dendrite
