...
def train_model(args, model, dataloader, criterion, optimizer, lr_scheduler, writer, device='cpu', val_dataloader=None, start_time=None): 
    ...
    wandb.init(project="HProb-UNet", config=vars(args))
    history = {
    ...
    val_images, val_truths = val_images[20:24], val_truths[0][20:24]
    ...
    for e in range(args.epochs):
        for mb, (images, truths) in enumerate(tqdm(dataloader)):
            ...
            wandb.log({
                "train/total_loss": loss.item() / args.pixels,
                "train/reconstruction_loss": criterion.last_loss['reconstruction_term'].item() / args.pixels,
                "train/kl_loss": criterion.last_loss['kl_term'].item() / args.pixels,
            ...
            if idx % args.val_period == 0 and val_dataloader is not None:
                ...
                with torch.no_grad():
                    ...
                    val_preds_bin = (torch.sigmoid(val_preds) > 0.5).float()
                    out_grid = make_grid(val_preds_bin, nrow=4, pad_value=val_preds.min().item())
        ...
        wandb.log({
                        "val/total_loss": mean_val_loss.item(),
                        "val/reconstruction_loss": mean_val_reconstruction_term.item(),
                        "val/kl_loss": mean_val_kl_term.item()
                    }, step=idx)
        lr_scheduler.step()
